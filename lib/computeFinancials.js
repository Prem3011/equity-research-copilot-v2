...
export function computeFinancials(fmpData) {
  ...
  const years = income.map((inc, i) => {
    ...
    const dividendsPaid = Math.abs(cf.netDividendsPaid || cf.dividendsPaid || cf.commonDividendsPaid || 0);
  ...
  const latestEps = years[years.length - 1]?.eps || 0;
  const peRatio = price && latestEps ? price / latestEps : 0;
  ...
  return {
    ...
    profile: {
      ...
      currencySymbol,
      isIndian,
      change: profile.change || profile.changes || 0,
      changePercent: profile.changePercentage || 0,
    },
    latest,
  };
}
